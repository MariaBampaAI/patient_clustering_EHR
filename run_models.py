...
def represent(AEmodels, modalities, cohort, datasets, model_run, epochs=100, path_results='RESULTS', pretrain=True):
    ...
    saved_models_dir = os.path.join(path_results, 'saved_models')
    losses_dir = os.path.join(path_results, 'losses')
    ...
    if pretrain==False:
        ...
        helpers.plot_model_loss(ae_val_loss, ae_train_loss, title, path_results)
    else:
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(f"{path}/losses"):

        os.makedirs(f"{path}/losses")

    if not os.path.isdir(f"{path}/saved_models"):
