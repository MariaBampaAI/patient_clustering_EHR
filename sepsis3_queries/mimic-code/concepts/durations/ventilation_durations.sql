  ...
  select
      ...
      , case
          ...
          when MechVent=1 then
           ...
           (EXTRACT(EPOCH FROM (CHARTTIME::timestamp - charttime_lag::timestamp)) / 3600)

            --EXTRACT(MINUTE from CHARTTIME::timestamp - charttime_lag::timestamp)/60
          else null
        end as ventduration
      ...
      , case
        -- if there is an extubation flag, we mark any subsequent ventilation as a new ventilation event
          ...
          --when CHARTTIME > DATETIME_ADD(charttime_lag::timestamp, INTERVAL '8' HOUR)
          when CHARTTIME > (charttime_lag::timestamp + INTERVAL '8' HOUR)
...
select icustay_id
  ...
  --, DATETIME_DIFF(max(charttime)::timestamp, min(charttime)::timestamp, 'MINUTE')/60 AS duration_hours
  ,(EXTRACT(EPOCH FROM (max(charttime)::timestamp - min(charttime)::timestamp)) / 3600) AS duration_hours
