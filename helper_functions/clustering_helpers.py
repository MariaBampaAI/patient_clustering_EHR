...
def to_3d_numpy(df, list_of_features):
    x = df.reset_index()
    ...
    vals = x[list_of_features].values
...
def calc_euclideans(df,list_of_features):
    X = df.reset_index()
    X_ = X[list_of_features]
...
def plot_clusters(dtw_dist, kmeans_labels, ward_labels, complete_labels, single_labels, true_labels, title):
    from sklearn.manifold import TSNE
    tsne = TSNE(perplexity=80, min_grad_norm=1E-12, n_iter=3000).fit_transform(dtw_dist)
    ...
    sns.scatterplot(x=tsne[:, 0],y=tsne[:, 1], hue=true_labels, ax=axes[0])
    axes[0].set_title("TSNE Original__{}".format(title))

    sns.scatterplot(x=tsne[:, 0],y=tsne[:, 1],hue=kmeans_labels, ax=axes[1])
    axes[1].set_title("Kmeans")

    sns.scatterplot(x=tsne[:, 0],y=tsne[:, 1],hue=single_labels, ax=axes[2])
    axes[2].set_title("Agglomerative (Single)")

    sns.scatterplot(x=tsne[:, 0],y=tsne[:, 1],hue=ward_labels, ax=axes[3])
    axes[3].set_title("Agglomerative (Ward)")

    sns.scatterplot(x=tsne[:, 0],y=tsne[:, 1],hue=complete_labels, ax=axes[4])
    ...
    axes[0].set_title("Original__{}{}".format(title,pca.explained_variance_ratio_))
    ...
    plt.tight_layout()
    plt.show()
    f.savefig('baseline1_window{}.png'.format(title))